...
f0 = 47.6; % GHz
num_f = size(f0,2); 
r = ones(size(f0))*2.2337;
phih = ones(size(f0))*(-0.6032);
z = ones(size(f0))*(0.3186);
% z = [-0.13 -0.14 -0.15 -0.16 -0.17];

theta = -30;%度
phi = -180;
k0 =2*pi.*f0*1e9/c;%cm???
kz0 = k0.*sin(theta/180*pi);
kr0 = k0.*cos(theta/180*pi).*cos(phi/180*pi);
nphi0 = k0.*sin(phi/180*pi).*r;

% [r,phih,z,kr_guess,nphi,kz,f]
% change kr_guess for possible multi-modes, i.e., O/X
yray0 = zeros(num_f,7);
for i = 1:num_f
    yray0(i,:) = [r(i),phih(i),z(i),-200,nphi0(i),kz0(i),f0(i)];
...
%% calculate power
jcalpower=0; % =1, calculate the power absorb; =0, only calculate tracing
if(jcalpower==1)
