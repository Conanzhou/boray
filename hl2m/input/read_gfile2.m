...
rho_bin = 0:0.005:1;
% densprof(:,1) = (-2.5 * rho_bin .^2 +2.5) * 1e19;%倒抛物线分布
load("ne_2m_h.mat","ne");
densprof(:,1) = ne * 1e6 -1.0e19;
densprof(:,2) = 0;
densprof(:,3) = 0;
...
% genray.dat
if(1==1)
nsrz0(1,:,:)=interp1(rho_bin,squeeze(densprof(:,1)),rhorz);
nsrz0(2,:,:)=interp1(rho_bin,squeeze(densprof(:,2)),rhorz);
nsrz0(3,:,:)=interp1(rho_bin,squeeze(densprof(:,3)),rhorz);
...
saveas(gcf,['gfile_eqdata_B0=',num2str(B0,3),'T,ni0=',num2str(n0,3),'.fig'])
